...
def buildTriblerOnWindows() {
    script{
        bat """
            ...
            python3 ./build/win/replace_nsi.py -r . %ARCH_TYPE_ARGUMENT%
    ...
    stages {
        ...
        stage('Build binaries') {
            parallel {
                stage('Build Win64') {
                    environment {
                        ARCH_TYPE_ARGUMENT = '--architecture x64'
                    }
                    when {
                        expression {
                ...
                stage('Build Win32') {
                    ...
                    environment {
                        ARCH_TYPE_ARGUMENT = '' // Leave empty for 32-bit
                    }
                    when {
                        expression {
